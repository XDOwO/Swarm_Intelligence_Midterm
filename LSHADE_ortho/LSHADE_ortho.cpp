...
class LSHADE {
  public:
    ...
    double evaluate(const gene_t &x) {
        --eval_amt;
        return calculate_test_function(matvec(basis, x).data(), dim, func_num);
    }
    ...
    double apply() {
        ...
        while (eval_amt > 0 && pop_size > min_pop_size) {
            ...
            for (int i = 0; i < pop_size && eval_amt > 0; ++i) {
                ...
                v = std::move(matvec(basis, v));
                for (auto &val : v) val = bound(val, 0);
                v = std::move(matvec(transpose(basis), v));
            ...
            int next_size = int(init_pop_size - (init_pop_size - min_pop_size) *
            ...
            if (generation % int(tot_eval / init_pop_size * 0.75) == 0 && bests.size() >= dim) {
